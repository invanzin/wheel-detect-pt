...
import cv2 # Mantido aqui, pois é usado diretamente no script

# Importar configurações de caminhos
from .config.paths import WHEEL_MODEL_PATH, OUTPUT_DIR, ensure_directories
from .utils.file_utils import create_directory, save_json_results
...
# A classe Wheel foi movida para src/core/models/wheel.py para melhor organização.
from .core.models.wheel import Wheel
# A classe Axle é mantida aqui com uma definição simplificada, pois sua lógica de
# 'is_lifted' é específica para a detecção baseada na linha do chão neste script,
# e difere do modelo Axle mais robusto em src/core/models/axle.py.
...
class Axle:
    ...
    is_dual: bool
    is_lifted: bool
    position: Tuple[int, int]  # posição média do eixo (x, y) para este script
    confidence: float
...
class WheelDetector:
    ...
    def __init__(
        ...
        if model_path is None:
            model_path = str(WHEEL_MODEL_PATH)
    ...
    def process_image(self, image_path: str, output_dir: str = None, show: bool = True) -> Dict:
        ...
        create_directory(output_path_dir)
        ...
        save_json_results(results, json_results_path)
...
def print_summary(results: Dict, image_path: str):
    """Imprime um resumo dos resultados da análise no console."""
    ...
    for axle in results.get('axle_details', []):
        print(
            ...
            f"Status: {axle['status']}"
        )

def handle_categorization(results: Dict, auto_categorize: bool):
    """Gerencia a categorização e movimentação dos arquivos de resultado."""
    if auto_categorize:
        print(f"\nArquivos salvos em: {Path(results['output_path']).parent}")
        print("(Categorização interativa desativada)")
        return

    selected_category = select_output_category()
    
    if selected_category and selected_category != "output":
        ...
        print(f"\nArquivos movidos para a categoria: '{selected_category}'")
        print(f"  - {', '.join(moved_files)}")
    else:
        print(f"\nArquivos mantidos na pasta de saída padrão: {Path(results['output_path']).parent}")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default="images/caminhao/sc_caminhao26_up.jpg",  # Usar / para compatibilidade entre OS
    ...
    try:
        ...
        results = detector.process_image(
            ...
            show=False  # Controlamos a exibição manualmente abaixo
        )

        # Imprimir resumo dos resultados no console
        print_summary(results, args.image_path)

        # Exibir a imagem anotada, se solicitado
        if not args.no_show:
            ...
            if annotated_img is None:
                ...
                print("\n[Visualização] Pressione qualquer tecla na janela da imagem para continuar...")
                cv2.waitKey(0)
                cv2.destroyAllWindows()

        # Lidar com a categorização e movimentação dos arquivos
        handle_categorization(results, args.auto_categorize)
